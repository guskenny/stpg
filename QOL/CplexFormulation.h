...
#include </opt/ibm/ILOG/CPLEX_Studio127/cplex/include/ilcplex/cplex.h>
...
namespace qol {
  ...
  private:
	...
	std::vector<int> primalStartIdx;
	std::vector<double> primalStartVal;
  public:
    ...
    virtual Status solveExact() {
	  ...
	  if(primalStartIdx.size()> 0){
		  std::vector<int> beg(2,0); beg[1]=primalStartIdx.size();
		  int effort = CPX_MIPSTART_AUTO;
		  CPXaddmipstarts(env,LP,1,primalStartIdx.size(),
						  beg.data(),primalStartIdx.data(),primalStartVal.data(),
						  &effort,0); // no name for start
		  primalStartIdx.clear(); primalStartVal.clear(); // clear memory
	  }
