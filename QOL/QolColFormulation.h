  ...
  public:
	...
	virtual ConstraintMIP addConstraint(const Row &row) {
      ...
      for (Index index = 0; index<row.lhs.size(); ++index) {
		  ...
		  const Index col=row.lhs[index].second;
		  if( matrix[col].empty() || matrix[col].back().second != row_index) {
			  matrix[col].push_back (CoeffIdxPair (row.lhs[index].first, row_index));
		  }else{ // add two entries for the same variable
			  matrix[col].back().first += row.lhs[index].first;
		  }
